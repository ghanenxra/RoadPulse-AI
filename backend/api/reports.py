...
def get_weekly_report(week: int = 4, zone: str = None, authority: str = None, db: Session = Depends(get_db)):
    ...
    return FileResponse(pdf_path, media_type='application/pdf', filename=f'roadpulse_weekly_w{week}.pdf')
...
def get_csv_export(type: str, week: int = None, db: Session = Depends(get_db)):
    ...
    return StreamingResponse(iterfile(), media_type="text/csv", headers={"Content-Disposition": f"attachment; filename={os.path.basename(file_path)}"})
